...
# Extraer y graficar IPC para cada configuración
for config_str, files in data_by_config.items():
    ...
    plt.ylabel("IPC")
    plt.title(f"Resultados de IPC para configuración {config_str}")
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()

    # Guardar gráfico en la carpeta de gráficos
    output_file = os.path.join(graphs_output_dir, f"IPC_{config_str}.png")
